...
function calculateBalances(items) {
  ...
  for (const movement of items) {
    ...
    if (movement.type === "expense") debitExpenses += amount;
    if (movement.type === "card_purchase") creditPurchases += amount;
    if (movement.type === "card_payment") cardPayments += amount;
  }
...
function render() {
  ...
  const totalCredit = Math.max(0, totals.card);
  const realBalance = totals.current - totalCredit;
  currentBalanceEl.textContent = formatCurrency(realBalance);
  cardBalanceEl.textContent = formatCurrency(totalCredit);
